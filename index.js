...
const routesList = require('./genetic/individual-static');
...
const compare = (individ1, individ2) => {
    return individ2.val - individ1.val;
};
...
const crossover = (path1, path2) => {
    ...
    for(let i = 0; i < path1.length; i++){
        child[path1[i]] = path2[i];
    }
    return child;
};

const mutation = (path, k) => {
    let arr1 = path.splice(0, k);
    let arr2 = path.splice(k+1, path.length);
    arr1 = arr1.reverse();
    return [...arr1, ...arr2];
};
...
const iteration = (population) => {
    ...
    const k4 = random(0, population.length);
    const k5 = random(0, CUSTOMERS);
    ...
    const newPopulation = selection(population);
...
const main = () => {
    ...
    console.log("The best found way: ", population[0].path);
